...
import { type CalendarEvent, type EventType, type EventFormData, type WorkingHoursConfig, DEFAULT_WORKING_HOURS } from "../../app/(protected)/calendar/types";
import {
  localizer,
  createSampleEvents,
  getDefaultEventForm,
...
export function CalendarClient({ workingHours }: CalendarClientProps) {
  // Calendar state
  const userTimezone = workingHours.timezone;
  const [events, setEvents] = useState<CalendarEvent[]>(() => createSampleEvents(userTimezone));
  ...
  const handleSelectSlot = useCallback(({ start, end }: { start: Date; end: Date }) => {
    const isInWorkingHours = isBookableSlot(start, workingHours);
  ...
  const slotPropGetter = useCallback((date: Date) => {
    if (isWorkingHour(date, workingHours)) {
  ...
  // Scroll to the start of working hours
  const scrollToTime = useMemo(() => {
    return setHours(setMinutes(new Date(), 0), Math.max(workingHours.start - 1, 0));
  }, [workingHours.start]);
